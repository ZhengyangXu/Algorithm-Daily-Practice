...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        ...
        ans = []
        def eatCan(candiesCount,query):
            favoriteType = query[0]
            favoriteDay = query[1]
            dailyCap = query[2]
            minConsume = favoriteDay + 1 
            maxConsume = (favoriteDay + 1) * dailyCap 
            beforeConsume = sum(candiesCount[:favoriteType])
            afterConsume = beforeConsume + candiesCount[favoriteType]
            if maxConsume < beforeConsume or minConsume > afterConsume: 
                return False 
            return True
        for query in queries:
            ans.append(eatCan(candiesCount,query))
