...
class Solution:
    def canMouseWin(self, grid: List[str], catJump: int,
                    mouseJump: int) -> bool:
        ...
        for mouse in range(total):
            ...
            for cat in range(total):
                ...
                for dx, dy in DIRS:
                    ...
                    while 0 <= row < rows and 0 <= col < cols and grid[row][
                            col] != '#' and jump <= mouseJump:
        ...
        results = [[[[0, 0], [0, 0]] for _ in range(total)]
        ...
        def getPrevStates(mouse: int, cat: int,
                          turn: int) -> List[Tuple[int, int, int]]:
            mouseRow, mouseCol = divmod(mouse, cols)
            catRow, catCol = divmod(cat, cols)
            ...
            for dx, dy in DIRS:
                ...
                while 0 <= i < rows and 0 <= j < cols and grid[i][
                        j] != '#' and jump <= maxJump:
        ...
        while q:
            ...
            for prevMouse, prevCat, prevTurn in getPrevStates(
                    mouse, cat, turn):
        ...
        return results[startMouse][startCat][MOUSE_TURN][
            0] == MOUSE_WIN and results[startMouse][startCat][MOUSE_TURN][
                1] <= MAX_MOVES
