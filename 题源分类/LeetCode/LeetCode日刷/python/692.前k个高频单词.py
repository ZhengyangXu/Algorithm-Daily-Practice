...
class Solution:
    ...
    def topKFrequent(self, words: List[str], k: int) -> List[str]:
        wordss = Counter(words)
        ans = []
        for word,count in wordss.items():
            heapq.heappush(ans,Word(word,count))
            if len(ans) > k:
                heapq.heappop(ans)
        
        ans.sort(reverse=True)
        return [word.word for word in ans]
...
# @lc code=end
    ...
    #     from collections import Counter
    #     count = Counter(words)
    #     ans = sorted(count,key = lambda x:(-count[x],x)) ##字典多条件排序
